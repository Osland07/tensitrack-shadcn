...
interface RulesProps {
    faktorRisiko: Record<string, string>;
}

export default function Rules({ faktorRisiko }: RulesProps) {
    const getFaktorNama = (kode: string): string => faktorRisiko[kode] || kode;
    const otherFaktorRisiko = Object.entries(faktorRisiko).filter(([code]) => code !== 'E01');
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-xs text-muted-foreground">
                                    Pengecekan gejala <span className="font-semibold">"{getFaktorNama('E01')}"</span>.
                                </p>
                            </div>
                        </div>
                        {/* Branch Cards */}
                        ...
                        <Card className="shadow-sm">
                            ...
                            <CardContent>
                                <ul className="text-sm list-decimal list-inside columns-1 sm:columns-2 gap-x-6">
                                    {otherFaktorRisiko.map(([kode, nama]) => (
                                        <li key={kode} className="mb-1">{nama}</li>
                                    ))}
                                </ul>
                            </CardContent>
                        </Card>
