...
export default function RiskFactors({ faktorRisiko, filters }: { faktorRisiko: Paginator<FaktorRisiko>, filters: { search?: string } }) {
    usePoll(2000);
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        AdminFaktorRisikoController.index.get({ query: { search } }); // Use Wayfinder for GET request with search query
    };

    return (
        <AppLayout breadcrumbs={[{ title: 'Faktor Risiko', href: AdminFaktorRisikoController.index.url() }]}>        
                ...
                <div className="flex justify-between items-center">
                    <div className="flex gap-2">
                        <Button asChild className="btn-modern bg-primary text-primary-foreground hover:bg-secondary hover:text-secondary-foreground">
                            <Link href={AdminFaktorRisikoController.create.url()}>Tambah</Link>
                        </Button>
                ...
                <div className="card p-6 md:p-8 space-y-6 min-h-[60vh]">
                    ...
                    <div className="rounded-md border overflow-x-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {faktorRisiko.data.map((faktorRisiko) => (
                                    <tr key={faktorRisiko.id} className="border-t">
                                        <td className="p-3 font-medium text-center">{faktorRisiko.kode}</td>
                                        <td className="p-3 text-center">{faktorRisiko.nama}</td>
                                                    ...
                                                    <Link href={AdminFaktorRisikoController.edit.url(faktorRisiko.id)}>Edit</Link>
                                                </Button>
                                                <AlertDialog>
                                                    ...
                                                    <AlertDialogContent>
                                                        ...
                                                        <AlertDialogFooter>
                                                            ...
                                                            <AlertDialogAction
                                                                onClick={() => {
                                                                    router.delete(AdminFaktorRisikoController.destroy.url(faktorRisiko.id), {
                                                                        preserveScroll: true,
                                                                    });
                                                                }}
                                                            >
                    ...
                    {/* Pagination Links */}
                    {faktorRisiko.links && faktorRisiko.links.length > 3 && (
                        <div className="flex justify-center mt-4 space-x-2">
                            {faktorRisiko.links.map((link, index) => (
                                <Link
                                    key={index}
                                    href={link.url || '#'}
