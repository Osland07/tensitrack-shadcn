...
export default function RiskLevels({ tingkatRisiko, filters }: { tingkatRisiko: Paginator<TingkatRisiko>, filters: { search?: string } }) {
    usePoll(2000);
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        AdminTingkatRisikoController.index.get({ query: { search } }); // Use Wayfinder for GET request with search query
    };

    return (
        <AppLayout breadcrumbs={[{ title: 'Tingkat Risiko', href: AdminTingkatRisikoController.index.url() }]}>        
                ...
                <div className="flex justify-between items-center">
                    <div className="flex gap-2">
                        <Button asChild className="btn-modern bg-primary text-primary-foreground hover:bg-secondary hover:text-secondary-foreground">
                            <Link href={AdminTingkatRisikoController.create.url()}>Tambah</Link>
                        </Button>
                ...
                <div className="card p-6 md:p-8 space-y-6 min-h-[60vh]">
                    ...
                    <div className="rounded-md border overflow-x-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {tingkatRisiko.data.map((tingkatRisiko) => (
                                    <tr key={tingkatRisiko.id} className="border-t">
                                        <td className="p-3 font-medium text-center">{tingkatRisiko.kode}</td>
                                        <td className="p-3 text-center">{tingkatRisiko.nama}</td>
                                        <td className="p-3 text-center">{tingkatRisiko.keterangan}</td>
                                        <td className="p-3 text-center">{tingkatRisiko.saran}</td>
                                                    ...
                                                    <Link href={AdminTingkatRisikoController.edit({ tingkatRisiko: tingkatRisiko.id }).url}>Edit</Link>
                                                </Button>
                                                <AlertDialog>
                                                    ...
                                                    <AlertDialogContent>
                                                        ...
                                                        <AlertDialogFooter>
                                                            ...
                                                            <AlertDialogAction
                                                                onClick={() => {
                                                                    router.delete(AdminTingkatRisikoController.destroy({ tingkatRisiko: tingkatRisiko.id }).url, {
                                                                        preserveScroll: true,
                                                                    });
                                                                }}
                                                            >
                    ...
                    {/* Pagination Links */}
                    {tingkatRisiko.links && tingkatRisiko.links.length > 3 && (
                        <div className="flex justify-center mt-4 space-x-2">
                            {tingkatRisiko.links.map((link, index) => (
                                <Link
                                    key={index}
                                    href={link.url || '#'}
