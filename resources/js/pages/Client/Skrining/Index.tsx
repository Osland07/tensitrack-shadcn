...
interface BackendResult {
    nama_risiko: string;
    penjelasan: string;
    keterangan_lengkap: string;
    saran: string;
}

export default function ScreeningIndex({ faktorRisiko }: ScreeningIndexProps) {
    ...
    const handleAnswer = async (answer: boolean) => {
        ...
        if (currentQuestionIndex < faktorRisiko.length - 1) {
            ...
            try {
                ...
                setBackendResults([{
                    nama_risiko: "Error",
                    penjelasan: "Terjadi kesalahan saat mengirim jawaban.",
                    keterangan_lengkap: "Pastikan Anda terhubung ke internet.",
                    saran: "Coba ulangi skrining."
                }]);
                setStep('results');
            } finally {
                setIsLoading(false);
    ...
    const progressValue = ((currentQuestionIndex + 1) / faktorRisiko.length) * 100;
        ...
        switch (step) {
                    ...
                    <Card className="w-full max-w-2xl">
                        ...
                        <CardContent className="py-8">
                            <p className="text-center text-2xl font-bold text-primary min-h-[100px] flex items-center justify-center">
                                {faktorRisiko[currentQuestionIndex].nama}
                            </p>
                        </CardContent>
            ...
            case 'results':
                ...
                const detectedFacts = Object.entries(userAnswersData)
                                            ...
                                            .map(([code]) => {
                                                const question = faktorRisiko.find(q => q.kode === code);
                                                return question ? question.nama : `Fakta ${code}`;
                                            });
                ...
                const riskColor = primaryResult ? riskColorClasses[primaryResult.nama_risiko] || "bg-gray-100" : "bg-gray-100";
                    ...
                    <Card className="w-full max-w-4xl">
                        ...
                        <CardContent className="p-6 overflow-y-auto">
                            ...
                            <div className={`text-center p-4 rounded-xl border-2 ${riskColor} mb-6`}>
                                <p className={`text-3xl font-bold mt-1`}>{primaryResult?.nama_risiko || 'Tidak Diketahui'}</p>
                                <p className={`mt-2 text-sm`}>{primaryResult?.penjelasan || 'Tidak ada penjelasan.'}</p>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="p-4 rounded-lg bg-muted">
                                    ...
                                    {primaryResult?.saran ? (
                                        <ul className="space-y-2">
                                            {primaryResult.saran.split('.').filter(s => s.trim() !== '').map((sug, index) => (
                                                <li key={index} className="flex items-start text-sm">
